...
@implementation DCDTBaseListController
    ...
    if (specifier) {
        ...
        for (int i = 0; i < rules.count; i++) {
            ...
            if ([opposingValue isKindOfClass:NSNumber.class]) {
                DCDTDependsSpecifierOperatorType operatorType = [self operatorTypeForString:[ruleComponents objectAtIndex:1]];
                ...
                if ([[requiredValue uppercaseString] isEqualToString:@"TRUE"] ||
                    ...
                    switch (operatorType) {
                        case DCDTEqualToOperatorType:
                            if ([opposingValue intValue] != [requiredValue boolValue])
                                shouldHide = TRUE;
                        break;

                        case DCDTNotEqualToOperatorType:
                            if ([opposingValue intValue] == [requiredValue boolValue])
                                shouldHide = TRUE;
                        break;

                        case DCDTGreaterThanOperatorType:
                            if ([opposingValue intValue] < [requiredValue boolValue])
                                shouldHide = TRUE;
                        break;

                        case DCDTLessThanOperatorType:
                            if ([opposingValue intValue] > [requiredValue boolValue])
                                shouldHide = TRUE;
                        break;

                        case DCDTGreaterThanOrEqualToOperatorType:
                            if ([opposingValue intValue] <= [requiredValue boolValue])
                                shouldHide = TRUE;
                        break;

                        case DCDTLessThanOrEqualToOperatorType:
                            if ([opposingValue intValue] >= [requiredValue boolValue])
                                shouldHide = TRUE;
                        break;

                        case DCDTBlankOperatorType:
                            shouldHide = FALSE;
                        break;
...
- (DCDTDependsSpecifierOperatorType)operatorTypeForString:(NSString *)string {
    //NSLog(@"[CMN_DEBUG] operatorTypeForString: %@", string);
    NSDictionary *operatorValues = @{
        @"==" : @(DCDTEqualToOperatorType),
        @"!=" : @(DCDTNotEqualToOperatorType),
        @">"  : @(DCDTGreaterThanOperatorType),          // In plist it needs to be &gt; because of stupid xml
        @"<"  : @(DCDTLessThanOperatorType),             // In plist it needs to be &lt; because of stupid xml
        @">=" : @(DCDTGreaterThanOrEqualToOperatorType), // In plist it needs to be &gt;= because of stupid xml
        @"<=" : @(DCDTLessThanOrEqualToOperatorType),    // In plist it needs to be &lt;= because of stupid xml
        @""   : @(DCDTBlankOperatorType)
    };
...
- (void)reset {
	...
	UIAlertAction *confirm = [UIAlertAction actionWithTitle:[self localizedStringForKey:@"RESET"] style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
		notify_post("com.dcproducts.dummytweak/Reset");
		notify_post("com.dcproducts.dummytweak/Respring");
    }];
...
- (void)bugReport {
	[[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"mailto:dtcalabro@gmail.com?subject=%5BiOS%20Tweak%20Support%5D%20Bug%20Report&body=Tweak%20name%3A%20DummyTweak%0D%0AMessage%3A%20I%20found%20a%20bug%20while%20using%20the%20DummyTweak%20tweak.%20The%20following%20explains%20how%20to%20reproduce%20the%20bug%20and%20includes%20any%20additional%20information%20I%20may%20have%20regarding%20the%20issue.%0D%0A%0D%0A"] options:@{} completionHandler:nil];
}

- (void)featureRequest {
	[[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"mailto:dtcalabro@gmail.com?subject=%5BiOS%20Tweak%20Support%5D%20Feature%20Request&body=Tweak%20name%3A%20DummyTweak%0D%0AMessage%3A%20I%20have%20a%20feature%20request%20for%20the%20DummyTweak%20tweak.%20The%20following%20explains%20my%20idea%20and%20any%20other%20input%20or%20information%20I%20may%20have%20regarding%20it.%0D%0A%0D%0A"] options:@{} completionHandler:nil];
}
