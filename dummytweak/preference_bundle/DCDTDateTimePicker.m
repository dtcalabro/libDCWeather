...
void updateValueLabelCallback(CFNotificationCenterRef center, void *observer, CFStringRef name, const void *object, CFDictionaryRef userInfo) {
    if (observer != nil)
        [(__bridge DCDTDateTimePicker *)observer updateValueLabel];
}

@implementation DCDTDateTimePicker
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(id)identifier specifier:(PSSpecifier *)specifier withPickerStyle:(DCDTDateTimePickerStyle)pickerStyle {
    ...
    if ([prefsDict objectForKey:[specifier propertyForKey:@"key"]] == nil) {
        if ([specifier propertyForKey:@"default"] != nil) {
            ...
            if (pickerStyle == DCDTDateTimePickerStyleTimeInterval) {
            ...
            } else if (pickerStyle == DCDTDateTimePickerStyleDate) {
            ...
            } else if (pickerStyle == DCDTDateTimePickerStyleTime) {
                [dateFormatter setDateFormat:@"hh:mm a"];
                NSDate *dateFromString = [dateFormatter dateFromString:[specifier propertyForKey:@"default"]];

                [prefsDict setObject:dateFromString forKey:[specifier propertyForKey:@"key"]];
            } else if (pickerStyle == DCDTDateTimePickerStyleDateAndTime) {
    ...
    if ([specifier propertyForKey:@"default"] != nil) {
        self.dateTimePickerView = [[DCDTDateTimePickerView alloc] initWithFrame:self.rootViewController.view.frame tintColor:self.themeTintColor key:[specifier propertyForKey:@"key"] defaultValue:[specifier propertyForKey:@"default"] defaults:[specifier propertyForKey:@"defaults"] postNotification:[specifier propertyForKey:@"PostNotification"] dateTimePickerStyle:pickerStyle];
    } else {
        self.dateTimePickerView = [[DCDTDateTimePickerView alloc] initWithFrame:self.rootViewController.view.frame tintColor:self.themeTintColor key:[specifier propertyForKey:@"key"] defaults:[specifier propertyForKey:@"defaults"] postNotification:[specifier propertyForKey:@"PostNotification"]];
    }
...
- (void)updateValueLabel {
    ...
    if (self.dateTimePickerStyle == DCDTDateTimePickerStyleTimeInterval) {
        ...
        if (self.dateTimePickerStyle == DCDTDateTimePickerStyleDate) {
            [formatter setDateFormat:@"MMMM d yyyy"]; // This is just a date
        } else if (self.dateTimePickerStyle == DCDTDateTimePickerStyleTime) {
            [formatter setDateFormat:@"hh:mm a"]; // This is 12-hour format
        } else if (self.dateTimePickerStyle == DCDTDateTimePickerStyleDateAndTime) {
